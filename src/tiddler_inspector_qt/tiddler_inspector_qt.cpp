    ...
    , pure_edit(new Tiddler_Pure_Edit)
{
    ...
    connect(store_handler, &Tiddlerstore_Handler::open_tiddler_model, this, [this, tiddler_view_edit_stack, view_index](Tiddler_Model* tiddlerModel) {
        pure_view->set_tiddler_model(tiddlerModel);
        if (tiddlerModel != nullptr) {
            connect(tiddlerModel, &Tiddler_Model::destroyed, tiddler_view_edit_stack, [tiddler_view_edit_stack, view_index]{
                tiddler_view_edit_stack->setCurrentIndex(view_index);
            });
        }
    });
